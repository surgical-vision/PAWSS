...
PatchHsvFeature::PatchHsvFeature(const Config &conf) :
    mHsvFeature(),
...
#if VERBOSE
    std::cout<<"Patch hsv histogram bins: "<< GetCount() << std::endl;
#endif
}


void PatchHsvFeature::PrepEval(const multiSample &samples)
{
...
void PatchHsvFeature::UpdateFeatureVector(const Sample &s)
{
