...
PatchHsvMFeature::PatchHsvMFeature(const Config &conf) :
    mHsvFeature(), mMotFeature(),
...
#if VERBOSE
    std::cout<<"Patch hsv mot histogram bins: "<< GetCount() << std::endl;
#endif

}

void PatchHsvMFeature::PrepEval(const multiSample &samples)
{
...
//            hists[bin + mRgbBinNum].at<float>(iy, ix) = 1;
            hists[bin + mHsvBinNum].at<float>(iy, ix) = mp[ix];
        }
    }
...
void PatchHsvMFeature::UpdateFeatureVector(const Sample &s)
{
    ...
    for(int pid=0; pid<mPatchNumX*mPatchNumY; ++pid)
    {
        ...
        if(mKernelType == Config::kKernelTypeIntersection)
        {
            ...
            fea_mot_sum += mFeatVec.segment(mBinNum*pid+mHsvBinNum, mMotBinNum).sum();
        }
        else if(mKernelType == Config::kKernelTypeLinear)
        {
            fea_hsv_sum += mFeatVec.segment(mBinNum*pid, mHsvBinNum).squaredNorm();
            fea_mot_sum += mFeatVec.segment(mBinNum*pid+mHsvBinNum, mMotBinNum).squaredNorm();
        }
    }
    ...
    if(mKernelType == Config::kKernelTypeLinear)
    {
        ...
        if(fea_mot_sum != 0) {
            for(int pid=0; pid<mPatchNumX*mPatchNumY; ++pid) {
                mFeatVec.segment(mBinNum*pid+mHsvBinNum, mMotBinNum) *= mMotWeight/(weight_norm * sqrt(fea_mot_sum)); }}
    }
    else if(mKernelType == Config::kKernelTypeIntersection)
    {
        ...
        if(fea_mot_sum != 0) {
            for(int pid=0; pid<mPatchNumX*mPatchNumY; ++pid) {
                mFeatVec.segment(mBinNum*pid+mHsvBinNum, mMotBinNum) *= mMotWeight/fea_mot_sum; }}
    }
}

void PatchHsvMFeature::UpdateWeightModel(const Sample &s)
{
...
void PatchHsvMFeature::setPrevImg(ImageRep &img)
{
